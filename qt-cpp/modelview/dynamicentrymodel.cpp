...
void
DynamicEntryModel::insert(int index, const QString &colorValue)
{
    ...
    // view protocol (início => manipulação => fim)
...
void
DynamicEntryModel::append(const QString &colorValue)
{
    insert(count(), colorValue);
}

void
DynamicEntryModel::remove(int index)
{
    if (index < 0 || index >= m_data.count()) {
        return;
    }
    ...
    emit endRemoveRows();
    // não esquecer de atualizar a propriedade "count"
    emit countChanged(m_data.count());
}

void
DynamicEntryModel::clear()
{
...
QColor
DynamicEntryModel::get(int index)
{
    if (index < 0 || index >= m_data.count()) {
        return QColor();
    }
...
int
DynamicEntryModel::rowCount(const QModelIndex &parent) const
{
    Q_UNUSED(parent);
    return m_data.count();
}
...
QVariant
DynamicEntryModel::data(const QModelIndex &index, int role) const
{
    int row = index.row();
...
DynamicEntryModel::roleNames() const
{
    return m_roleNames;
}
